...
def get_json_data(user_address, api_key):
...
def get_lat_long(data):
       ...
       if data['status'] == 'OK':#if status is okay, continue with operation
            ...
            if results:#if the results it recieves is not none
                location = results[0]['geometry']['location']#This accesses location dict from the first result in the list
...
def sms_reply():
    ...
    if user_phone_number not in user_state:
        ...
        data = get_json_data(user_address, api_key)
